#![allow(
    unused_imports,
    unused_variables,
    unused_mut,
    dead_code,
    unused_assignments,
    unused_comparisons
)]
use pce::emulator::Emulator;
use std::error::Error;
use std::io::Write;

fn main() -> Result<(), Box<dyn Error>> {
    let rom = std::fs::read("roms/Kato-chan & Ken-chan (Japan).pce")?;
    let mut emu = Emulator::new();
    emu.load_hucard(&rom)?;
    emu.reset();

    let mut frames = 0;
    let mut last_frame: Option<Vec<u32>> = None;
    while frames < 100 {
        emu.tick();
        if let Some(f) = emu.take_frame() {
            last_frame = Some(f);
            frames += 1;
        }
    }

    let pixels = last_frame.as_ref().unwrap();
    let width = 256;
    let height = pixels.len() / width;

    // Save PNG-compatible PPM
    let mut data = Vec::with_capacity(width * height * 3);
    for &p in pixels {
        data.push(((p >> 16) & 0xFF) as u8);
        data.push(((p >> 8) & 0xFF) as u8);
        data.push((p & 0xFF) as u8);
    }
    let header = format!("P6\n{} {}\n255\n", width, height);
    let mut file = std::fs::File::create("frame_100.ppm")?;
    file.write_all(header.as_bytes())?;
    file.write_all(&data)?;
    println!("Saved frame_100.ppm");

    // Check text rows with detailed output
    println!("\n=== Text at Y=172-179 (HISCORE) frame 100 ===");
    for y in 172..180.min(height) {
        print!("Y={:3}: ", y);
        for x in 48..220 {
            let idx = y * width + x;
            let p = pixels[idx];
            let r = (p >> 16) & 0xFF;
            let g = (p >> 8) & 0xFF;
            let b = p & 0xFF;
            if r == 0 && g == 0 && b == 0 {
                print!(" ");
            } else if r > 200 && g > 200 && b > 200 {
                print!("W");
            } else if r > 0xD0 {
                print!("w");
            }
            // light
            else if r == 0x24 && g == 0x24 {
                print!(".");
            }
            // dark
            else if r < 0x40 && g < 0x40 && b < 0x40 {
                print!("d");
            }
            // dark
            else {
                print!("{}", (p & 0xF) as u8);
            } // lower nibble
        }
        println!();
    }

    println!("\n=== Y=204-211 (PUSH RUN BUTTON) ===");
    for y in 204..212.min(height) {
        print!("Y={:3}: ", y);
        for x in 48..220 {
            let idx = y * width + x;
            let p = pixels[idx];
            let r = (p >> 16) & 0xFF;
            let g = (p >> 8) & 0xFF;
            let b = p & 0xFF;
            if r == 0 && g == 0 && b == 0 {
                print!(" ");
            } else if r > 200 && g > 200 && b > 200 {
                print!("W");
            } else if r > 0xD0 {
                print!("w");
            } else if r < 0x40 && g < 0x40 && b < 0x40 {
                print!("d");
            } else {
                print!("{}", (p & 0xF) as u8);
            }
        }
        println!();
    }

    // Check CR and font status
    let cr = emu.bus.vdc_register(0x05).unwrap_or(0);
    println!(
        "\nCR: {:04X} (BG={}, sprites={})",
        cr,
        (cr >> 7) & 1,
        (cr >> 6) & 1
    );

    // Check font integrity at frame 100
    let base = 0x148 * 16;
    let w0 = emu.bus.vdc_vram_word(base as u16);
    println!(
        "Font 'H' row 0: {:04X} (expected 0x0066, plane 0 = {:02X})",
        w0,
        w0 & 0xFF
    );

    Ok(())
}
