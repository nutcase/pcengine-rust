...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Disassemble around the two addresses
    for start_addr in [0xE2C0u16, 0xE360] {
        println!("\nDisassembly from ${:04X}:", start_addr);
    ...
    // Also show MPR mapping
    println!("\nMPR bank mapping:");
    for i in 0..8 {
        let bank = emu.bus.mpr(i);
        println!(
            "  MPR{}: bank ${:02X} -> address ${:04X}-${:04X}",
            i,
            bank,
            i * 0x2000,
            (i + 1) * 0x2000 - 1
